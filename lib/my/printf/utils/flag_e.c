...
static int my_elen(long double nb, int precision)
{
    ...
    len += precision + 2;
    len += 2 + my_nbrlen(exponent, 10) + (exponent < 10);
    return len;
}
    ...
    long double nb_ld, int newsum)
{
    ...
    int nb_len = my_elen(ABS(is_long ? nb_ld : nb_d), flaginfo->precision);
...
{
    my_print_flag_e(flaginfo, list, sum);
}
